...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '50935279754@s.whatsapp.net'; // ton numÃ©ro ou le groupe cible
...
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
      *Golden Naruto Crew Bot Connected Successfully* ðŸš€
â• â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 50935279754; // mettez votre numÃ©ro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '35279754' };
                    configmanager.saveP();
                ...
                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['50935279754@s.whatsapp.net'], // emplace par ton numÃ©ro WhatsApp 
